...
class REINFORCE:
    ...
    def __init__(self, obs_space_dims: int, action_space_dims: int, model_state: dict = None, optimizer_state: dict = None):
    ...
    def states(self):
        return self.net.state_dict(), self.optimizer.state_dict()
...
total_num_episodes = int(10e3)  # Total number of episodes
...
for seed in [8]:  # Fibonacci seeds
    ...
    if continue_training:
        checkpoint = torch.load("checkpoints/episode_4001.pt", weights_only=True)
        begin_episode = checkpoint["episode"]
        agent = REINFORCE(obs_space_dims, action_space_dims, checkpoint["model_state"], checkpoint["optimizer_state"])
    else:
        begin_episode = 0
    ...
    for episode in tqdm(range(begin_episode, total_num_episodes)):
        ...
        if episode % 1000 == 0:
            ...
            save_video(wrapped_env.render(), f"video_{episode}_{avg_reward}", fps=env.metadata["render_fps"])
            model, optimizer = agent.states()
            torch.save(
                {
                    "model_state": model,
                    "optimizer_state": optimizer,
                    "episode": episode+1,
                    "total_episodes": total_num_episodes
                }, 
                f"checkpoints/episode_{episode+1}.pt"
            )
